...
class User
{
	...
	{	
		try {
			$this->con = new PDO('mysql:host=localhost;dbname=phpfirebase', 'admin', 'connect123');
			$this->con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    		$this->con->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
			// This assumes that you have placed the Firebase credentials in the same directory
			// as this PHP file.
			$serviceAccount = ServiceAccount::fromJsonFile(__DIR__.'/chatsystem-416c7-firebase-adminsdk-f1cte-afa1e82a60.json');
	...
	public function createAccount($fullname, $username, $email, $password, $image){
		...
		if (empty($fullname) || empty($username) || empty($email) || empty($password)) {
			...
			$stmt = $this->con->prepare("INSERT INTO `users`(`uuid`, `fullname`, `username`, `email`, `password`, `image`) VALUES (:uuid, :fullname, :username , :email, :password, :image)");
	...
	public function loginUser($username, $password){
		...
		if ($stmt->rowCount() == 1) {
			...
			if (password_verify($password, $row['password'])) {
				...
				$stmt1 = $this->con->prepare("UPDATE users SET status='1' WHERE id='".$row['id']."'");
				$stmt1->execute();
	...
	public function getUsers(){
		$query = $this->con->query("SELECT uuid,fullname,username,status,last_login,image FROM users");
		$ar = [];
	...
	public function logout(){
		...
		if (isset($_SESSION['username'])) {
			...
			$stmt1 = $this->con->prepare("UPDATE users SET status='0',last_login='".date("Y-m-d")."' WHERE username='".$_SESSION['username']."'");
			$stmt1->execute();
